...
> 現在は、街のデータモデル・住める街レベルの判定・決定論的なレイアウト生成・検証（役場検査）と、それを取り出すCLI / HTTP APIに加え、承認済み78素材だけで構築した一続きの街を歩くゲーム画面があります。旧市街から石橋と階段を通って雪地区の道場を調べ、街へ戻って森林の家へ向かう初回導線を画面内で案内します。素材を読み込めない場合は理由を表示して停止し、旧素材や代替描画へは戻りません。78点はプロジェクト所有者がChatGPT Proで生成した素材として制作来歴と採用を確認済みです。権利・法令適合性をソフトウェアが自動保証するものではありません。完成した因果関係を証明するツールでも、署名済みのMacアプリでもありません。
...
ブラウザには `/api/town` を読み取る街ゲームが開きます。矢印キー・WASD・クリックで旧市街、雪地区、港、森林を連続して歩き、橋、階段、施設入口、17の調査地点を訪れられます。施設では観測・推測・未確認の根拠を分けて確認し、同じ場所から街へ戻れます。初回は画面内の4段階ガイドが道場と森林の家まで案内し、その後は自由に探索できます。承認済みAsset Forge exportが無い／読み込めない場合は理由を表示して停止します。生のJSONは次のエンドポイントから確認できます。

Asset Forgeの定義は合計110件で、現在のゲームに必要な78件は承認・export済み、残り32件は任意の将来拡張です。公開schema v2 manifestを唯一の入口として、17建物、22人物、19地形、18小物、2効果の全78点を、到達可能な街または調査地点で意味を持つよう決定的に配置します。キャラクターは検証済みの方向・動作セルを、エフェクトは宣言済みフレームを切り抜いて描画します。公開契約が揃っていることは、画像の美観や利用権まで自動判定したという意味ではありません。
