...
> 現在は、街のデータモデル・住める街レベルの判定・決定論的なレイアウト生成・検証（役場検査）と、それを取り出すCLI / HTTP APIに加え、Canvas上を矢印キーまたはWASDで歩ける小さな街画面があります。ゲームに必要なAsset Forge素材78点を、承認台帳とschema v2 manifest付きで同梱しています。画像を読み込めない場合は理由を表示して手続き描画へ戻ります。78点はプロジェクト所有者がChatGPT Proで生成した素材として制作来歴と採用を確認済みです。権利・法令適合性をソフトウェアが自動保証するものではありません。完成した因果関係を証明するツールでも、署名済みのMacアプリでもありません。
...
ブラウザには `/api/town` を読み取る街画面が開きます。地図へフォーカスして矢印キーまたはWASDで移動でき、建物の選択、住みやすさ、観測・推測・不明の証拠区分、接続者ギルドを確認できます。公開素材の読込状況は画面に明示され、承認済みAsset Forge exportが無い／読み込めない場合は手続き描画を使います。生のJSONは次のエンドポイントから確認できます。

Asset Forgeの定義は合計110件で、現在のゲームに必要な78件は承認・export済み、残り32件は任意の将来拡張です。公開schema v2 manifestを唯一の入口として、道路・水辺・橋・建物状態・NPC・小物を街の文脈から決定的に選びます。キャラクターは検証済みの方向・動作セルを、エフェクトは宣言済みフレームを切り抜いて描画します。公開契約が揃っていることは、画像の美観や利用権まで自動判定したという意味ではありません。
