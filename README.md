...
> **現在の製品状態（2026-07-15）:** 静的検査、街モデル、決定論的レイアウト、loopback API、承認済み78素材の公開経路、Canvas上の移動・施設訪問・根拠表示までは技術検証済みです。一方、現在のゲーム画面は `/api/city` のfile/import graphと `/api/town` の生成レイアウトを街の地理へ反映しておらず、リポジトリが変わっても世界の大部分が固定です。現行78素材も完成に必要な量・種類が大幅に不足し、固定配置・反復・静止NPCなどの使い方も不合格です。ユーザーのゲームデザイン審査には不合格であり、完成品または完成候補ではありません。設計担当向けの正確な現在地、制約、未達、Asset Forge能力は [Fable5DesignHandoff.md](Fable5DesignHandoff.md) を正本とします。
...
ブラウザには `/api/town` を読み取る技術検証用の街ゲームが開きます。矢印キー・WASD・クリックで旧市街、雪地区、港、森林を連続して歩き、橋、階段、施設入口、17の調査地点を訪れられます。施設では観測・推測・未確認の根拠を分けて確認し、同じ場所から街へ戻れます。初回は画面内の4段階ガイドが道場と森林の家まで案内し、その後は自由に探索できます。ただし、この固定導線と固定地理はFable5の再設計対象であり、現状を製品仕様として確定したものではありません。承認済みAsset Forge exportが無い／読み込めない場合は理由を表示して停止します。生のJSONは次のエンドポイントから確認できます。

Asset Forgeの定義は合計110件で、現在のゲームに必要な78件は承認・export済み、残り32件は任意の将来拡張です。公開schema v2 manifestを唯一の入口として、17建物、22人物、19地形、18小物、2効果の全78 IDをruntimeから参照します。これはID・byte・寸法・来歴・bindingの技術網羅であり、到達可能なsceneでの意味ある使用、構図、ゲーム性を証明するものではありません。キャラクターは検証済みの方向・動作セルを、エフェクトは宣言済みフレームを切り抜いて描画します。公開契約が揃っていることは、画像の美観や利用権まで自動判定したという意味でもありません。
