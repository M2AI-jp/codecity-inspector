...
> 現在は、街のデータモデル・住める街レベルの判定・決定論的なレイアウト生成・検証（役場検査）と、それを取り出すCLI / HTTP APIに加え、Canvas上を矢印キーまたはWASDで歩ける小さな街画面があります。ゲームに必要なAsset Forge素材78点を、承認台帳とschema v2 manifest付きで同梱しています。画像を読み込めない場合は理由を表示して手続き描画へ戻ります。画像の美観とライセンスの最終判断は自動化しません。完成した因果関係を証明するツールでも、署名済みのMacアプリでもありません。
