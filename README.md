...
> **現在の製品状態（2026-07-17）:** `/` では、承認済み78素材と `/api/town/legacy` を使う従来の街をプレイできます。これは移動・施設訪問・根拠表示まで確認できる技術検証版で、地理と導線の多くは固定です。`/fable5-v2/` には、リポジトリから生成した街と最初の調査ツアーを使うFable5候補版を分離してありますが、Wave Aの全109素材が人間承認・export・trust pinされるまでは理由を表示して停止します。旧78素材へのfallbackや新旧素材の混在は行いません。したがって、現時点のFable5は完成品またはリリース候補ではありません。設計担当向けの正確な現在地、制約、未達、Asset Forge能力は [Fable5DesignHandoff.md](Fable5DesignHandoff.md) を正本とします。
