...
import {
  ...
  writeCandidate,
  writeReport
} from '../../../studio/art-department/index.mjs';

const repositoryRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..');

test('custody registry contains 22 immutable originals and current bytes pass the gate', () => {
...
test('factory writes are limited to candidates and reports; no promotion surface exists', async () => {
  ...
  assert.equal('promoteAsset' in await import('../../../studio/art-department/index.mjs'), false);
});
