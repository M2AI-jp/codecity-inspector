...
> **LEGACY 18×12 DIAGNOSTIC — 2026-07-18 SUPERSEDED**
>
> 本書の座標、mask、overscan、camera proofは candidate01/02 の失敗原因を再現する履歴証拠としてのみ残す。
> 製造・collision・door・camera・runtime の正本ではない。新しい正本は
> `Fable5VerticalSlice24x16.md` と `old-town-exterior-master-v1.png`。本書の座標を新画像へ流用してはならない。
...
- カメラ(整数 zoom、2026-07-18 target-town 優先のユーザー決定で導入と操作画面を分離):
  - 制作マスターは市庁舎の全高用に論理 y=-2.5 tiles まで北 overscan を持つ。
  - **establishing**: zoom 1。1920×1080 はoverscanを含む全18×14.5 tilesを中央表示し、庁舎・広場・主要道・前景を
    1画面で見せる。1280×720 は論理worldの上下0.375 tileだけをcropし、同じ三層構図を維持する。
  - **gameplay**: zoom 2。exterior spawn `(2,7)` を必ず画面内に置き、1920×1080 TL `(0,3)`、
    1280×720 TL `(0,4)` から追従開始する。player足元のscreen位置はそれぞれ `(320,640)` / `(320,512)`。
  - establishingは最大1200ms。入力をlockしたままgameplay cameraへeaseし、完了後にだけ操作を開始する。
    最初の方向入力またはEscapeは導入をskipし、方向入力はbufferしてcamera完了直後に適用する。
  - player の移動後は論理worldとoverscanを含む境界でpixel-snapしてclampする。旧 TL `(1,2)` / `(2,2)` は
    市庁舎の屋根・時計をほぼ全て切るため廃止。
  - framing証拠: `art/production/vertical-slice/qa/camera-establishing-*-zoom1-v3.png` と
    `art/production/vertical-slice/qa/camera-gameplay-*-zoom2-v3.png`。
...
|--------|------|---------------|----------|
| char_player | cutaway証拠 `(2,6)` / exterior初期spawn `(2,7)` | idle north | inn counter(対話可能距離)。操作開始前にgameplay camera内で足元可視 |
