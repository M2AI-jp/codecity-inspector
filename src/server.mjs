...
const DEFAULT_GAME_PATH = '/fable5-v2/';
...
function isValidTownPayload(payload, inspection) {
  if (!hasExactKeys(payload, ['schemaVersion', 'repository', 'habitability', 'facts', 'worldPlan'])) {
    return false;
  }
  if (payload.schemaVersion !== 2 || !Array.isArray(payload.facts)) return false;
  ...
  return validation.ok === true
    ...
    && JSON.stringify(validation) === JSON.stringify(payload.worldPlan?.validation);
}
...
export function parseCliArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (argument === '--open') result.open = true;
      ...
      result.repoPath = path.resolve(argument.slice('--repo='.length));
...
export async function startServer({
  ...
  if (!(await lstat(actualPublicRoot)).isDirectory()) {
    throw new Error('Public root must be a directory');
  }
  ...
  const server = createServer(async (request, response) => {
    ...
    if (pathname === '/api/town') {
      try {
        ...
        if (!isValidTownPayload(payload, inspection)) {
    ...
    if (result.status !== 200) {
      sendText(
        response,
        result.status,
        result.status === 404 ? 'Not found' : 'Request rejected',
        headOnly
      );
      return;
    }
    ...
    response.setHeader(
      'Content-Type',
      CONTENT_TYPES.get(path.extname(result.filePath).toLowerCase()) ?? 'application/octet-stream'
    );
