...
`Fable5PrefabSpec.md` と `tools/asset-forge/contracts/`、品質参照は
`art/references/target-town.png`、データモデルの正本は
`src/town/world-plan-schema.mjs` である。矛盾があれば「ArtContract §10 > PrefabSpec > 本書 > 実装の現状」の順で解決し、
...
- 本書の素材名は snake_case の**正規名**(例: `tree_a`, `snowcap_xl`)。実装前に
  `Fable5PrefabSpec.md` の新IDへ1:1で確定すること。旧ID、hash ID、類似IDへのaliasや
  自動読み替えは禁止。未対応の正規名が1つでも残ればビルドエラーにする。
...
footprint は正規値(タイル)。PrefabSpecまたはArtContractと食い違えば上位文書を優先し本表を更新する。
...
## 13. 実装順序(ArtContract §10による改訂)

1. `Fable5VerticalSlice.md` の18×12座標を128px/tileでブロックアウトする。
2. 1枚の旧市街マスターボードへ清書し、そこから57 prefabを切り出す。
3. N1–N9とAC-1〜AC-8を通し、prefabだけで同一座標へ再組み立てする。
4. Leadと所有者がVertical Sliceを実機承認する。
5. 承認後にだけ6画面を新しい道幅3の座標で再発行し、generator一般化へ進む。

旧6画面の先行レンダリング、旧asset-id-map、旧assetの再利用は禁止。
